...
const Game = () => {
  ...
  const handleSolve = () => {
    ...
    setHintsTaken((hints: any) => hints + newHints);
    setIsPlayerWon(true);
  ...
  const handleHint = () => {
    ...
    // Adding hint count
    setHintsTaken((hints: any) => hints + 1);
...
console.log(maxEmptyCellsCount);
  ...
  const handleCellClick = (row:any, column:any, isModifiable: any) => {
    if (!isModifiable) {
      animateElement(".grid-table", "headShake");
      return;
    }

    console.log({row,column, isModifiable});
    

   // moves registered when the value is not 0
    if (clickValue !== 0) setMovesTaken((moves: any) => moves + 1);
  ...
  //  else 
   if (Object.keys(grid).length == 0 && Object.keys(startingGrid).length == 0 ) handleNewGame(gameMode);
